...
class KPValidationToast {
    ...
    func showToastOnStatusBar(message: String, color: UIColor = AvatrecColor.green) -> ValidationToast {
        ...
        if _statusBarHeight > 20{
            ...
            height = message.heightWithConstrainedWidth(width: _screenSize.width - 20, font: UIFont.LHTFontWith(.poppinsRegular, size: 15)) + (45)
        }else{
            toast = UINib(nibName: "ValidationToast", bundle: nil).instantiate(withOwner: nil, options: nil)[0] as? ValidationToast
            toast.setToastMessage(message: message)
            toast.animatingView.backgroundColor = color
            height = message.heightWithConstrainedWidth(width: _screenSize.width - 20, font: UIFont.LHTFontWith(.poppinsRegular, size: 15)) + (3)
        }
    ...
    func showStatusMessageForInterNet(message: String, withColor color: UIColor = AvatrecColor.red) -> ValidationToast {
...
class ValidationToast: UIView {
    ...
    class func showStatusMessage(message: String, yCord: CGFloat = _statusBarHeight, inView view: UIView? = nil, withColor color: UIColor = AvatrecColor.red) -> ValidationToast {
        let toast = UINib(nibName: "ValidationToast", bundle: nil).instantiate(withOwner: nil, options: nil)[0] as! ValidationToast
        let strHeight = message.heightWithConstrainedWidth(width: _screenSize.width - 20, font: UIFont.LHTFontWith(.poppinsRegular, size: 15)) + (10 * _widthRatio)
        toast.layoutIfNeeded()
    ...
    func setToastMessage(message: String) {
        let font = UIFont.LHTFontWith(.poppinsRegular, size: 16.widthRatio)
