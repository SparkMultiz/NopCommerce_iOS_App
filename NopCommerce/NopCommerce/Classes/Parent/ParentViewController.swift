...
func getLocalizedKey(str: String) -> String {
    return arrLang.first{$0.name.isEqual(str: str)}?.value ?? str
}
...
class ParentViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    func setDefaultUI() {
        ...
        collectionView?.semanticContentAttribute = _appDelegator.isArabic ? .forceRightToLeft : .forceLeftToRight
...
extension ParentViewController {
    ...
    func showError(data: Any?, view: UIView?) {
        ...
        if let dict = data as? NSDictionary{
            if let arrValidation = dict["ValidationMessage"] as? [String], arrValidation.count > 0 ,let msg = arrValidation.first {
    ...
    func showSuccMsg(dict: NSDictionary) {
        JTValidationToast.show(message: dict.getStringValue(key: "Message"), viewController: self, type: .success, presentType: .top, animationStlye: .slide, completion: {
...
extension ParentViewController {
    ...
    func getUserProfile(block: @escaping (Bool) -> ()) {
        KPWebCall.call.getUserProfileData { (json, statusCode) in
...
extension ParentViewController {
    ...
    func postWishList() {
        _defaultCenter.post(name: .addToWishList, object: nil)
    }
    
    func postCart() {
        _defaultCenter.post(name: .addToCart, object: nil)
    }
...
extension ParentViewController {
    ...
    func hideShowHomeTabbar(isHidden: Bool){
        if let tabBarController = self.tabBarController, tabBarController is JTTabBarController{
